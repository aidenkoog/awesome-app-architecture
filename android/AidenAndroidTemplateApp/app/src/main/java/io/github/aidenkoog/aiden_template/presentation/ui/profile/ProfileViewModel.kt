...
constructor(
    ...
    val selectedCategory: MutableState<ProfileCategory?> = mutableStateOf(null)
    ...
    init {
        ...
        savedStateHandle.get<ProfileCategory>(STATE_KEY_SELECTED_CATEGORY)?.let { c ->
            setSelectedCategory(c)
        }

        if(recipeListScrollPosition != 0){
            onTriggerEvent(ProfileEvent.RestoreStateEvent)
        }
        else{
            onTriggerEvent(ProfileEvent.NewSearchEvent)
        }
    ...
    fun onTriggerEvent(event: ProfileEvent){
        viewModelScope.launch {
            try {
                when(event){
                    is ProfileEvent.NewSearchEvent -> {
                        newSearch()
                    }
                    is ProfileEvent.NextPageEvent -> {
                        nextPage()
                    }
                    is ProfileEvent.RestoreStateEvent -> {
                        restoreState()
                    }
                }
            }catch (e: Exception){
    ...
    fun onSelectedCategoryChanged(category: String) {
        val newCategory = getProfileCategory(category)
    ...
    private fun setSelectedCategory(category: ProfileCategory?){
