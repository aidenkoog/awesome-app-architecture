...
import io.github.aidenkoog.aiden_template.presentation.ui.insights.InsightsCategory
import io.github.aidenkoog.aiden_template.presentation.ui.insights.getInsightsCategory
...
constructor(
    ...
    val selectedCategory: MutableState<InsightsCategory?> = mutableStateOf(null)
    ...
    init {
        ...
        savedStateHandle.get<InsightsCategory>(STATE_KEY_SELECTED_CATEGORY)?.let { c ->
            setSelectedCategory(c)
        }

        if(recipeListScrollPosition != 0){
            onTriggerEvent(InsightsEvent.RestoreStateEvent)
        }
        else{
            onTriggerEvent(InsightsEvent.NewSearchEvent)
        }

    }

    fun onTriggerEvent(event: InsightsEvent){
        viewModelScope.launch {
            try {
                when(event){
                    is InsightsEvent.NewSearchEvent -> {
                        newSearch()
                    }
                    is InsightsEvent.NextPageEvent -> {
                        nextPage()
                    }
    ...
    fun onSelectedCategoryChanged(category: String) {
        val newCategory = getInsightsCategory(category)
    ...
    private fun setSelectedCategory(category: InsightsCategory?){
