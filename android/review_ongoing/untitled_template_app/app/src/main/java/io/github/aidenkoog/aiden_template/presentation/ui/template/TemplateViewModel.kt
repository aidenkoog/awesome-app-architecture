...
constructor(
    ...
    val selectedCategory: MutableState<TemplateCategory?> = mutableStateOf(null)
    ...
    init {
        ...
        savedStateHandle.get<TemplateCategory>(STATE_KEY_SELECTED_CATEGORY)?.let { c ->
            setSelectedCategory(c)
        }

        if(recipeListScrollPosition != 0){
            onTriggerEvent(TemplateEvent.RestoreStateEvent)
        }
        else{
            onTriggerEvent(TemplateEvent.NewSearchEvent)
        }

    }

    fun onTriggerEvent(event: TemplateEvent){
        viewModelScope.launch {
            try {
                when(event){
                    is TemplateEvent.NewSearchEvent -> {
                        newSearch()
                    }
                    is TemplateEvent.NextPageEvent -> {
                        nextPage()
                    }
                    is TemplateEvent.RestoreStateEvent -> {
                        restoreState()
                    }
                }
            }catch (e: Exception){
    ...
    fun onSelectedCategoryChanged(category: String) {
        val newCategory = getTemplateCategory(category)
    ...
    private fun setSelectedCategory(category: TemplateCategory?){
