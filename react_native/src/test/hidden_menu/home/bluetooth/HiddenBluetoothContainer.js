...
import { BATTERY_CHARACTERISTIC_UUID, BATTERY_SERVICE_UUID, RX_CHARACTERISTIC_UUID, SERVICE_UUID, TX_CHARACTERISTIC_UUID } from '../../../../utils/ble/BleConfig'
...
const HiddenBluetoothContainer = ({ }) => {
    ...
    const [macAddressErrorHintMessage, setMacAddressErrorHintMessage] = useState("")
    ...
    const {
        ...
        executeReconnectBleDeviceUseCase
    } = ConnectBleUseCase()
    ...
    function addLogMessageHandler(logMessageToAdd) {
        ...
        setLogMessages((currentLogMessages) => [
            ...currentLogMessages,
            { text: "[" + getCurrentTime() + "]" + logMessageToAdd, id: Math.random().toString() },
        ])
    }
    ...
    onPressTestCase = (itemId) => {
        ...
        switch (itemId) {
            case 'bt_t01':
                addLogMessageHandler(">>> execute Auto-Test\n---------------------------------------------------\n")
                executeAutoTestCase()
                break
            ...
            case 'bt_t0':
                executeGetBatteryLevelUseCase().then((batteryLevel) => {
                    addLogMessageHandler("<<< Succeeded to get battery level of device")
            ...
            case 'bt_t9':
                ...
                executeSetMtuUseCase(512).then((mtu) => {
    ...
    getWelcomeMessage = (batteryLevel) => {
        const welcomeMessage =
            ...
            + "2. [BLE Connection]: " + bleConnectionCompleteState + "\n"
            + "3. [TX Enable]: " + bleTxUuidNotificationState + "\n"
            + "4. [Flow-Control Enable]: " + bleFlowControlUuidNotificationState + "\n"
            + "5. [BAS Enable]: " + bleBatteryUuidNotificationState + "\n"
            + "6. [Device Name]: " + bleDeviceName + "\n"
            + "7. [OS]: " + (Platform.OS === 'android' ? "Android" : "iOS") + "\n"
            + "8. [Device MAC(UUID)]: " + bleMacOrUuid + "\n\n"
            + "------------------- UUIDs -------------------\n"
            + "1. [Service]: " + SERVICE_UUID + "\n"
            + "2. [RX]: " + RX_CHARACTERISTIC_UUID + "\n"
            + "3. [TX]: " + TX_CHARACTERISTIC_UUID + "\n"
            + "4. [BAS Service]: " + BATTERY_SERVICE_UUID + "\n"
            + "5. [BAS]: " + BATTERY_CHARACTERISTIC_UUID + "\n"
            + "6. [Flow-Control]: " + BATTERY_CHARACTERISTIC_UUID + "\n"
            + "---------------------------------------------------\n"
    ...
    handleOkPopup = () => {
        ...
        if (deviceName.length < 15 || deviceName == "" || deviceMacAddress < 12 || deviceMacAddress == "") {
        ...
        addLogMessageHandler(">>> trying to reconnect ble device...\n"
            + "1. Device Name to connect: " + deviceName + "\n"
            + "2. MAC Address to connect: " + macAddressWithColon)

        executeReconnectBleDeviceUseCase(deviceName, macAddressWithColon).catch((e) => {
            outputErrorLog(LOG_TAG, e + " occurred by executeReconnectBleDeviceUseCase")
     ...
     */
    onDeviceMacAddressChanged = (value) => {
        ...
        if (deviceMacAddress == "") {
            ...
            if (deviceMacAddress.length < 12) {
                setMacAddressErrorHintMessage("Please enter 12 digits without colon(:).")
                setOkButtonDisable(true)
            } else {
                setMacAddressErrorHintMessage("")
                setOkButtonDisable(false)
